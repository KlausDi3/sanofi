...
export default function AnalysisPage() {
  ...
  // Results live on the backend keyed by job id, so a reload (or arriving back
  // from /results) can restore them instead of forcing a re-run — which would
  // cost another pass over the corpus.
  useEffect(() => {
    ...
    getJobStatus(jobId)
      .then((status) => {
        if (cancelled || status.status !== "completed" || !status.result) return;
        setResults(status.result);
      })
      .catch(() => {
        // A job the backend no longer knows about (restarted process, expired
        // run) is not an error worth showing — the page is usable without it.
      });
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (result.id) {
        rememberJobId(result.id);
        syncJobIdToUrl(result.id);
      }
    } catch (err) {
        ...
        <div className="p-10 space-y-8">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-4">
              ...
              {/* The metadata views live on their own page; without a pointer
                  here nothing tells the user they exist. */}
              {results && (
                <Link
                  href={withJobId("/results", results.id)}
                  className="flex items-center justify-between px-5 py-4 bg-[var(--card)] border border-[var(--border)] rounded-lg hover:border-[var(--primary)] transition-colors group"
                >
                  <div>
                    <p className="font-primary text-sm font-medium text-[var(--foreground)]">
                      Break these themes down by metadata
                    </p>
                    <p className="font-secondary text-xs text-[var(--muted-foreground)] mt-0.5">
                      See how themes vary across the columns in your dataset
                    </p>
                  </div>
                  <BarChart3 className="w-5 h-5 text-[var(--muted-foreground)] group-hover:text-[var(--primary)] transition-colors" />
                </Link>
              )}
            </div>
          </div>
